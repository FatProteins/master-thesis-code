...
for (( j=0; j<CLUSTER_SIZE-1; j++ )); do
  ...
  ALL_ETCD_CLIENT_PORTS="${ALL_ETCD_CLIENT_PORTS}$((ETCD_CLIENT_BASE_PORT + j)),"
done

ALL_ETCD_CLIENT_PORTS="${ALL_ETCD_CLIENT_PORTS}$((ETCD_CLIENT_BASE_PORT + j))"
export ALL_ETCD_CLIENT_PORTS
DISCOVERY_PORT=$((ETCD_GRPC_BASE_PORT + j))
ETCD_INITIAL_CLUSTER="${ETCD_INITIAL_CLUSTER}etcd_instance_$j=http://${HOST_IP}:${DISCOVERY_PORT}"
...
for (( i=0; i<CLUSTER_SIZE; i++ )); do
  ...
  export DA_EDUCATION_HOST_PORT=$((8080 + i))
...
#  if [[ "$i" -lt 4 ]]; then
    docker compose -p $PROJECT_NAME -f "${LOCAL_DIR}/docker-compose-local.yml" -f "${LOCAL_DIR}/docker-compose-education.yml" up -d da
