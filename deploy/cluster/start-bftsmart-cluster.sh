...
if [ -z "${DISABLE_INTERRUPT}" ]; then
  ...
  echo "Only starting bftsmart instances, because interrupts are disabled."
fi
bash -c "${COMPOSE_COMMAND} -p ${PROJECT_NAME} -f ${REMOTE_DEPLOYMENT_DIR}/docker-compose-cluster.yml --env-file ${REMOTE_DEPLOYMENT_DIR}/.env-cluster up -d bftsmart"
