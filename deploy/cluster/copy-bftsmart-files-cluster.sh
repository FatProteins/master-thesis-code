...
do
  ...
  "--skip-da-image-upload")
    ...
    ;;
  "--skip-bftsmart-image-upload")
    SKIP_BFTSMART_IMAGE_UPLOAD=1
    ;;
  "--from-host")
    shift
    FROM_HOST="$1"
...
if [ -z "${SKIP_IMAGE_UPLOAD}" ]; then
  if [ -z "${FROM_HOST}" ]; then
    ...
    if [ -z "${SKIP_BFTSMART_IMAGE_UPLOAD}" ]; then
      docker save -o "${TEMP_DEPLOYMENT_DIR}/bftsmart-image.tar" "${BFTSMART_IMAGE_NAME}:${BFTSMART_IMAGE_VERSION}"
      scp "${TEMP_DEPLOYMENT_DIR}/bftsmart-image.tar" "${TARGET_USER}@${TARGET_HOST}:${REMOTE_DEPLOYMENT_DIR}"
    else
      echo "Skipping bftsmart Image upload"
    fi
  else
    ...
    if [ -z "${SKIP_BFTSMART_IMAGE_UPLOAD}" ]; then
      ssh -t -o StrictHostKeyChecking=accept-new "${FROM_USER}@${FROM_HOST}" "scp ${REMOTE_DEPLOYMENT_DIR}/bftsmart-image.tar ${TARGET_USER}@${TARGET_HOST}:${REMOTE_DEPLOYMENT_DIR}"
    else
      echo "Skipping bftsmart Image upload"
    fi
  fi
  ...
  if [ -z "${SKIP_BFTSMART_IMAGE_UPLOAD}" ]; then
    ssh "${TARGET_USER}@${TARGET_HOST}" "docker load -i ${REMOTE_DEPLOYMENT_DIR}/bftsmart-image.tar"
  fi

else
  echo "Skipping upload of bftsmart image"
fi
